...
@Component
public class GetRowByDistanceAction implements ActionInterface {
    ...
    @Autowired
    public GetRowByDistanceAction(ElementDao elementDao, EnhancedElementService elementService,
    ...
    public Object doAction(ActionBoundary action) {
        ...
        if (Boolean.TRUE.equals(action.getActionAttributes().containsKey("distance"))) {
            return this.entityConverter
                    .convertFromEntity(this.elementDao.findByParents_elementId_AndLngBetweenAndActiveIsTrue(
                            LongAndStringConverter.convertToLong(element.getElementId()),
                            (double) action.getActionAttributes().get("distance") - 0.5,
                            (double) action.getActionAttributes().get("distance") + 0.5));
        }
        return new NotFoundException("row not found in requested distance.");
    }
}